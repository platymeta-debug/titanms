...
class MapTimer: public Timers::TimerHandler {
public:
	void setMapTimer(int mapid){
		if(ctimer.find(mapid) != ctimer.end())
			...
			return;
		Maps::mapTimer(mapid);
		timers[Timers::timer->setTimer(10000, this)] = mapid;
		ctimer[mapid] = 1;
	}
...
hash_map <int,int> MapTimers::timers;
hash_map <int,int> MapTimers::ctimer;
MapTimer* Maps::timer;
