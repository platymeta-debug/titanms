...
private:
	...
	bool flip;
public:
	MobKillDelay(Map* map, int mobid, Position pos, bool flip):map(map), mobid(mobid), pos(pos), flip(flip){}
	void run(){
		map->getMobs()->summonFromMob(mobid, pos.x, pos.y, flip);
	}	
};
...
void MapMobs::checkSpawn(bool timer){
	...
	if(timer){
		...
		for(int i=0; i<(int)smobs->size(); i++){
			...
			else if(last[i] + (*smobs)[i]->getDelay() > (int)GetTickCount()) continue;
...
void MapMobs::kill(Mob* mob, Player* by){
	...
	//Timers::getInstance()->startTimer(DataProvider::getInstance()->getMobSummonDelay(mob->getMobID()), new MobKillDelay(map, mob->getMobID(), mob->getPosition(), mob->getStance()%2==1)); 
	summonFromMob(mob->getMobID(), mob->getPosition().x, mob->getPosition().y, mob->getStance()%2==1);
	for(int i=0; i<(int)mobs.size(); i++){
		if(mobs[i] == mob){
...
void MapMobs::summonFromMob(int mobid, int x, int y, bool flip){
	...
	for(int i=0; i<(int)summons->size(); i++){
		spawnMob((*summons)[i], x, y, -1, true, 0, flip);
	}
}
Mob* MapMobs::spawnMob(int mobid, int x, int y, int orgID, bool spawneffect, int fh, bool flip, bool find){
