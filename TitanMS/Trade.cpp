...
bool Trade::invite(Player* to){
	if(to->getTrade() != NULL){
		player->send(PacketCreator().showMassage("The other player is already trading with someone else.", 5));
		return false;
	}
	invited = to;
...
void Trade::decline(){
	player->send(PacketCreator().showMassage(invited->getName() + " has declined your trade request", 5));
}
void Trade::accept(){
